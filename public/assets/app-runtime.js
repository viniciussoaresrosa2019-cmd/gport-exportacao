    ...
    (() => {
      ...
      const toViewClient = c => ({ id: c.id, nome: c.name, cnpj: c.tax_id, contato: c.contact, telefone: c.phone, email: c.email, pais: c.country, endereco: c.address, rucManual: c.ruc_manual === true, dueOnly: c.due_only === true, active: c.active !== false });
      ...
      const toApiProcess = p => {
        ...
        const deadline = dateForDatabase(p.prazo), shippingDate = dateForDatabase(p.envio), containerCollectionDate = dateForDatabase(p.coleta), dueIssueDate = dateForDatabase(p.dueEmissao);
        const rucManual = client?.rucManual === true, dueOnly = client?.dueOnly === true;
        if (!dueOnly && !deadline) throw new Error('Informe o deadline de draft no formato dd/mm.');
      ...
      open = p => {
        ...
        renderClientOptions();
        form.elements.exportador.value = p.clientId || clients.find(client => client.nome === p.exportador)?.id || '';
        form.elements.prazo.value = dateForField(p.prazo);
      ...
      el('clientForm').onsubmit = async e => {
        e.preventDefault(); const v = Object.fromEntries(new FormData(e.currentTarget)); const payload = { name:v.nome, taxId:v.cnpj, contact:v.contato, phone:v.telefone, email:v.email, country:v.pais, address:v.endereco, rucManual:v.rucManual === 'on', dueOnly:v.dueOnly === 'on' };

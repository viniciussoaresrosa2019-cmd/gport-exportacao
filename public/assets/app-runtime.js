    ...
    (() => {
      ...
      const renderVgm = () => {
        ...
        el('vgmList').innerHTML = vgmProcesses.length ? `<table class="data-table vgm-table"><thead><tr><th>BOOKING</th><th>EXPORTADOR / IMPORTADOR</th><th>VGM ENVIADO?</th><th>DATA DO ENVIO</th><th>ENVIADO PARA</th><th>PROCESSO FÍSICO COM</th><th>DEADLINE DE AGENDAMENTO</th><th>DEADLINE DE LIBERAÇÃO</th></tr></thead><tbody>${vgmProcesses.map(p => `<tr class="process-status-row ${p.canalLiberacao ? `process-channel-${String(p.canalLiberacao).toLowerCase()}` : ''}"><td data-label="Booking"><strong>${esc(p.booking || '—')}</strong></td><td data-label="Exportador / Importador">${esc(p.exportador || '—')}<span class="sub">${esc(p.importador || '')}</span></td><td data-label="VGM enviado"><select data-vgm-status="${p.id}" ${canEdit ? '' : 'disabled'}>${options(p)}</select></td><td data-label="Data do envio">${esc(p.dataEnvioVgm || '—')}</td><td data-label="Enviado para"><input data-vgm-sent-to="${p.id}" value="${esc(p.vgmEnviadoPara || '')}" ${canEdit ? '' : 'readonly'}></td><td data-label="Processo físico com"><select data-vgm-analyst="${p.id}" ${canEdit ? '' : 'disabled'}>${analystOptions(p)}</select></td><td data-label="Deadline de agendamento"><input data-vgm-release-schedule="${p.id}" value="${esc(p.agendamentoLiberacao || '')}" placeholder="dd/mm hh:mm" ${canEdit ? '' : 'readonly'}></td><td data-label="Deadline de liberação"><input data-vgm-release-deadline="${p.id}" value="${esc(p.deadlineLiberacao || '')}" placeholder="dd/mm hh:mm" ${canEdit ? '' : 'readonly'}></td></tr>`).join('')}</tbody></table>` : '<div class="empty">Nenhum processo cadastrado.</div>';
        if (canEdit) {

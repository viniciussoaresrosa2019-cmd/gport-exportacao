    ...
    (() => {
      ...
      const renderVgm = () => {
        ...
        el('vgmList').innerHTML = vgmProcesses.length ? `<table class="data-table vgm-table"><thead><tr><th>BOOKING</th><th>EXPORTADOR / IMPORTADOR</th><th>STATUS VGM</th><th>ENVIO</th><th>DESTINO</th><th>RESPONSÁVEL</th><th>AGENDAMENTO</th><th>LIBERAÇÃO</th></tr></thead><tbody>${vgmProcesses.map(p => `<tr class="process-status-row ${p.canalLiberacao ? `process-channel-${String(p.canalLiberacao).toLowerCase()}` : ''}"><td data-label="Booking"><strong>${esc(p.booking || '—')}</strong></td><td data-label="Exportador / Importador">${esc(p.exportador || '—')}<span class="sub">${esc(p.importador || '')}</span></td><td data-label="Status VGM"><select aria-label="Status VGM do booking ${esc(p.booking || '')}" data-vgm-status="${p.id}" ${canEdit ? '' : 'disabled'}>${options(p)}</select></td><td data-label="Data do envio">${esc(p.dataEnvioVgm || '—')}</td><td data-label="Enviado para"><input aria-label="VGM enviado para do booking ${esc(p.booking || '')}" data-vgm-sent-to="${p.id}" value="${esc(p.vgmEnviadoPara || '')}" ${canEdit ? '' : 'readonly'}></td><td data-label="Processo físico com"><select aria-label="Responsável pelo processo físico do booking ${esc(p.booking || '')}" data-vgm-analyst="${p.id}" ${canEdit ? '' : 'disabled'}>${analystOptions(p)}</select></td><td data-label="Deadline de agendamento"><input aria-label="Deadline de agendamento do booking ${esc(p.booking || '')}" data-vgm-release-schedule="${p.id}" value="${esc(p.agendamentoLiberacao || '')}" placeholder="dd/mm hh:mm" ${canEdit ? '' : 'readonly'}></td><td data-label="Deadline de liberação"><input aria-label="Deadline de liberação do booking ${esc(p.booking || '')}" data-vgm-release-deadline="${p.id}" value="${esc(p.deadlineLiberacao || '')}" placeholder="dd/mm hh:mm" ${canEdit ? '' : 'readonly'}></td></tr>`).join('')}</tbody></table>` : '<div class="empty">Nenhum processo cadastrado.</div>';
        if (canEdit) {

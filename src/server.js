...
const validRoles = ['admin', 'analyst', 'vgm', 'financeiro'];
...
app.patch('/api/processes/:id/vgm', authenticate, vgmManagerOnly, asyncRoute(async (req, res) => {
  ...
  const result = await query(
    'UPDATE processes SET vgm_status=$1, physical_process_analyst=$2 WHERE id=$3 RETURNING *',
    [vgmStatus, physicalProcessAnalyst, req.params.id]
  );
  if (!result.rowCount) return res.status(404).json({ error: 'Processo não encontrado.' });
  await audit(req.user.sub, 'process.vgm_updated', 'process', req.params.id, { vgmStatus, physicalProcessAnalyst });
  res.json(result.rows[0]);
}));
